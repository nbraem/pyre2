...
class ReTests(unittest.TestCase):
    ...
    def test_weakref(self):
        ...
        x = re.compile(b'ab+c')
        y = proxy(x)
        self.assertEqual(x.findall(b'QabbbcR'), y.findall(b'QabbbcR'))

    def test_search_star_plus(self):
        self.assertEqual(re.search(b'x*', b'axx').span(0), (0, 0))
        self.assertEqual(re.search(b'x*', b'axx').span(), (0, 0))
        self.assertEqual(re.search(b'x+', b'axx').span(0), (1, 3))
        self.assertEqual(re.search(b'x+', b'axx').span(), (1, 3))
        self.assertEqual(re.search(b'x', b'aaa'), None)
        self.assertEqual(re.match(b'a*', b'xxx').span(0), (0, 0))
        self.assertEqual(re.match(b'a*', b'xxx').span(), (0, 0))
        self.assertEqual(re.match(b'x*', b'xxxa').span(0), (0, 3))
        self.assertEqual(re.match(b'x*', b'xxxa').span(), (0, 3))
        self.assertEqual(re.match(b'a+', b'xxx'), None)
    ...
    def test_basic_re_sub(self):
        ...
        self.assertEqual(re.sub(b'(?P<unk>x)', b'\g<unk>\g<unk>', b'xx'), b'xxxx')
        self.assertEqual(re.sub(b'(?P<unk>x)', b'\g<1>\g<1>', b'xx'), b'xxxx')

        self.assertEqual(re.sub(b'a',b'\\t\\n\\v\\r\\f\\a\\b\\B\\Z\\a\\A\\w\\W\\s\\S\\d\\D',b'a'),
                         b'\t\n\v\r\f\a\b\\B\\Z\a\\A\\w\\W\\s\\S\\d\\D')
        self.assertEqual(re.sub(b'a', b'\t\n\v\r\f\a', b'a'), b'\t\n\v\r\f\a')
        self.assertEqual(re.sub(b'a', b'\t\n\v\r\f\a', b'a'),
                         (chr(9)+chr(10)+chr(11)+chr(13)+chr(12)+chr(7)).encode('utf-8'))

        self.assertEqual(re.sub(b'^\s*', b'X', b'test'), b'Xtest')

    def test_bug_449964(self):
        # fails for group followed by other escape
        self.assertEqual(re.sub(b'(?P<unk>x)', b'\g<1>\g<1>\\b', b'xx'),
                         b'xx\bxx\b')
    ...
    def test_bug_1661(self):
        ...
        self.assertRaises(ValueError, re.match, pattern, b'A', re.I)
        self.assertRaises(ValueError, re.search, pattern, b'A', re.I)
        self.assertRaises(ValueError, re.findall, pattern, b'A', re.I)
        self.assertRaises(ValueError, re.compile, pattern, re.I)

    def test_bug_3629(self):
        # A regex that triggered a bug in the sre-code validator
        re.compile(b"(?P<quote>)(?(quote))")
